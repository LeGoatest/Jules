...
> ## NOTE:
>Create a new folder `project_docs/` in the root directory of the project then create a sub folder `spec/` and `steering/`, Then you will need to read each file in the sub folder `spec/` and `steering/`, following the guidelines in each file make new files related to the project and place them in the new `project_docs/` folder in there corresponding folders (`spec/` and `steering/`). Lastly take a clean copy of agents.md and place it in the root of `project_docs/`.
